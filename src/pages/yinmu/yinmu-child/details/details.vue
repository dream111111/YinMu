<template>
	<div class="diantaiList">
		<img :src="imgUrl" alt=""/>
		<router-link tag='span' class="back" :to="{ name: 'yinmu', params: { selected: '4' }}"></router-link>
		<div class="content">
			<div class="left">
				<p>播放全部</p>
			</div>
...
export default {
  ...
  created(){
  	...
  	var currentUrl = this.HOST+"/yinmu/bangdan.json";
  	this.$axios.get(currentUrl).
  		then(res =>{
  			var str = this.$route.path;
  	 		var str1 = str.substring(str.length-1);
  	 		this.result = res.data.result[str1].music;
  	 		this.imgUrl = res.data.result[str1].imgUrl;
  		})
  		.catch(error =>{
  			console.log(error);
  		})
