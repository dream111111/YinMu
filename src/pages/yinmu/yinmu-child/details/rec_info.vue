<template>
	<div class="regebang">
		<img :src="img" alt="" />
		<router-link tag='span' class="back" to="/index/yinmu"></router-link>
		<div class="content">
			<div class="left">
				<p>播放全部</p>
			</div>
...
<script>
	export default{
		...
		created:function(){
			...
			var log=str[str.length-1];
			this.address=this.$route.path;
			if(log=="intro"){
				var currentUrl = this.HOST+"/yinmu/intro.json";
				...
				this.$axios.get(currentUrl)
				.then(res => {
					this.img=res.data.result[parseInt(log)].url;
					this.listData=res.data.result[parseInt(log)].music;
				})
				.catch(error => {
					console.log(error)
				})
